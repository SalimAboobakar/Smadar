...
export const calculatePurchasingPowerFactor = (
  region,
  projectType,
  targetAudience
) => {
  ...
  const rawFactor =
    ...
    employmentFactor *
    priceAcceptance *
    economics.purchasingPowerIndex *
...
export const calculateEnhancedWeightedDemand = (
  region,
  projectType,
  targetAudience
) => {
  const regionData = dhofarRegions[region] || dhofarRegions.salalah; // fallback to salalah if region not found
  const projectData = projectTypes[projectType] || projectTypes.hotels; // fallback to hotels if projectType not found
  const audienceData =
    targetAudiences[targetAudience] || targetAudiences.tourists; // fallback to tourists if targetAudience not found
  const economics = economicIndicators[region] || economicIndicators.salalah;
  ...
  const purchasingPowerFactor = calculatePurchasingPowerFactor(
    region,
    projectType,
    targetAudience
  );
...
export const calculateEnhancedGovernanceFactor = (projectType, region) => {
  ...
  const infrastructureBonus =
    (regionData.investmentData?.infrastructureScore || 7) / 10;
  ...
  const adjustedGovernance =
    baseGovernance * approvalUncertainty * infrastructureBonus;

  return Math.round(adjustedGovernance * 1000) / 1000;
};

// حساب مؤشر الجاذبية الاستثمارية المحسن (IAI) - مضمون عدم تجاوز 100%
export const calculateEnhancedIAI = (
  region,
  projectType,
  targetAudience,
  ...
  const demand = calculateEnhancedWeightedDemand(
    region,
    projectType,
    targetAudience
  );
  const governance = calculateEnhancedGovernanceFactor(projectType, region);
  const purchasingPower = calculatePurchasingPowerFactor(
    region,
    projectType,
    targetAudience
  );
  ...
  const seasonalityImpact =
    projectData.seasonalityRisk * audienceData.seasonality;
  ...
  const weightedScore =
    ...
    0.1 * normalizedSeasonality;

  // 4. تطبيق عوامل التعديل الواقعية المحسنة
  const realismFactors = {
    marketRisk: 0.8 + economics.marketMaturity * 0.1, // مخاطر أقل في الأسواق الناضجة
...
export const calculateEnhancedSS = (
  projectType,
  investmentAmount = 100000,
  region
) => {
  ...
  const adjustedROI =
    baseROI * economicGrowthFactor * inflationAdjustment * marketMaturityBonus;
  ...
  const totalRisk =
    baseRisk * marketVolatility + economicRisk + competitionRisk;
  ...
  const adjustedCost =
    baseCost * costOfLivingAdjustment * (1 - efficiencyFactor * 0.2);

  // العوائق التنظيمية المحسنة
  const baseHurdles =
    (projectData.permitsRequired * projectData.avgApprovalDays) / 365;
  const bureaucracyFactor = 1.1 - economics.marketMaturity * 0.2; // بيروقراطية أقل
  const infrastructureBonus =
    (regionData.investmentData?.infrastructureScore || 7) / 10;
  const adjustedHurdles =
    baseHurdles * bureaucracyFactor * (1 - infrastructureBonus * 0.3);
...
export const getEnhancedTopRisks = (projectType, targetAudience, region) => {
  ...
  if (economics.unemploymentRate > 0.1) {
    risks.push({
      ...
      level: "high",
      description: `معدل بطالة مرتفع (${(
        economics.unemploymentRate * 100
      ).toFixed(1)}%) قد يقلل الطلب المحلي`,
    });
  }
  ...
  if (economics.inflationRate > 0.025) {
    risks.push({
      ...
      level: "medium",
      description: `معدل تضخم (${(economics.inflationRate * 100).toFixed(
        1
      )}%) قد يرفع التكاليف التشغيلية`,
    });
  }
  ...
  if (projectData.seasonalityRisk > 0.3) {
    ...
    risks.push({
      ...
      description: `تقلبات موسمية قوية (${(seasonalImpact * 100).toFixed(
        1
      )}%) - خطط للتدفق النقدي`,
    });
  }
...
export const calculateEnhancedConfidenceRate = (
  iai,
  ss,
  projectType,
  region
  ...
  return Math.max(0.25, Math.min(0.9, confidence));
};
